...
- **Enable a real Content-Security-Policy instead of disabling it.** Helmet was configured with `contentSecurityPolicy: false` (flagged by CodeQL as `js/insecure-helmet-configuration`, high severity) on the rationale that the demo's React client renders with inline styles a strict CSP would block. That rationale did not hold: this process is an API/image server — every route returns image bytes or JSON and it never serves the client's HTML, which Vite serves from a separate origin — and a CSP on an image or JSON response does not constrain the page embedding it. Disabling it bought nothing and cost a real security header. The policy is now scoped to what this server actually emits: `default-src 'none'` denies everything by default, `img-src 'self' data:` covers the image responses, and `frame-ancestors`/`base-uri`/`form-action` are pinned to `'none'` so a response can never be reframed or used as a form target. (`server/src/app.ts`)
